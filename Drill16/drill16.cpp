...
private:
    ...
    void color_change(Color c) { lines.set_color(c); }
    void style_change(Line_style l) { lines.set_style(l); }

    void show_color_menu();
    void hide_color_menu();
    void show_style_menu();
    void hide_style_menu();

    void red_pressed() { color_change(Color::red); hide_color_menu(); }
    void blue_pressed() { color_change(Color::blue); hide_color_menu(); }
    void black_pressed() { color_change(Color::black); hide_color_menu(); }
    void color_menu_pressed() { color_menu_button.hide(); show_color_menu(); }

    void solid_pressed() { style_change(Line_style(Line_style::solid, 3)); hide_style_menu(); }
    void dash_pressed() { style_change(Line_style(Line_style::dash, 3)); hide_style_menu(); }
    void dot_pressed() { style_change(Line_style(Line_style::dot, 3)); hide_style_menu(); }
...
Lines_window::Lines_window(Point xy, int w, int h, const string& title)
    ...
    style_menu_button(Point(x_max()-80,120),80,20,"style menu",cb_style_menu)
{
    attach(next_button);
    attach(quit_button);
    attach(next_x);
    ...
    lines.set_style(Line_style(Line_style::solid, 3));
    attach(lines);
}

void Lines_window::show_color_menu()
{
...
void Lines_window::show_style_menu()
{
